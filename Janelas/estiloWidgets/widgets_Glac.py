...
class Entry(ttk.Entry):
    def __init__(self, master=None):
        super().__init__(master)
        self.configure(style="EntryStyle.TEntry", font=('Calibri', 10, 'bold'))

class ButtonGlac(atk.Button3d):
    def __init__(self, parent, texto="texto", comando="comando", msgbal=False,
                 fundo='#456E96', **kwargs):
        atk.Button3d.__init__(self, parent, bg=fundo, **kwargs)
        self.configure(text=texto)
        self.configure(command=comando)
        self.bind("<Motion>", self.selection)
        self.msgbal = msgbal

    def selection(self, event):
        if self.msgbal == False:
            pass
        else:
            root = Toplevel()
            Frame = atk.Frame3d(root, '#333333')
            Frame.place(relx=-0.05, rely=-0.05, relwidth=1.1, relheight=1.1)
            self.lb = Label(root, bd=1, bg='gray90', fg='gray25')
            self.lb.configure(text=self.msgbal)
            self.lb.place(relx=0.01, rely=0.5, relwidth=0.98, relheight=0.35)
            self.lb2 = Label(root, bd=1, bg='gray90', fg='brown', font=('Verdana', 9, 'bold'))
            self.lb2.configure(text='Legenda:')
            self.lb2.place(relx=0.01, rely=0.2, relwidth=0.98, relheight=0.3)
            root.overrideredirect(True)
            root.geometry("390x62+600+45")
