...
class Aba2:
    def aba2(self):
        ...
        botaoAddServicos2.place(relx=0.935, rely=0.11, relwidth=0.05, relheight=0.11)

        # treeview configs
        style = ttk.Style()
        style.theme_use('default')
        style.configure("Treeview.Heading", font=('Arial', 10, 'bold'), background="gray40",
            foreground='gray80')
        style.configure("Treeview", background="gray20", foreground="gray50",
            fieldbackground="gray30", font=('Calibri', 10))
        style.map('Treeview', background=[('selected', '#7499b8')], foreground=[('selected', 'gray20')])
        style.configure("Vertical.TScrollbar", background="#D1D1D1")
        style.configure("TNotebook", background="gray40", foreground="gray60",
                        fieldbackground="gray50", font=('Arial', 10, 'bold'))
        style.configure("TOptionMenu", background="gray40", foreground="gray60",
                        fieldbackground="gray50")
        style.map('TNotebook', background=[('selected', '#F5B041')],
                  foreground=[('selected', 'gray20')])
        style.element_create("plain.field", "from", "clam")

        style.configure("EntryStyle.TEntry", background="gray25", foreground="gray60",
            fieldbackground="gray25", fieldforeground="gray25")
        ##

        self.barraServProd = ttk.Scrollbar(self.frame_aba2, orient='vertical',
