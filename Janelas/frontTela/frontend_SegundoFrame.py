...
class SegundoFrame:
    def segundo_frame(self):
        ...
        btAdd.place(relx=0.72, rely=0.22, relwidth=0.145, relheight=0.27)

        # Botão Limpa
        btLimp = ButtonGlac(self.FrameCliente, self.m_Limpar, self.limpa_cliente)
        btLimp.place(relx=0.86, rely=0.22, relwidth=0.145, relheight=0.27)

        # Label data
        descrData = LabelGlac(self.FrameCliente, self.m_Data)
        descrData.configure(bg='gray20')
        descrData.place(relx=0.7, rely=0.04, relwidth=0.1, relheight=0.15)

        # Entrada Dia
        self.entradaDataorc = DateEntry(self.FrameCliente, locale="pt_BR")
        self.entradaDataorc.place(relx=0.8, rely=0.04, relwidth=0.18, relheight=0.15)
        ...
        lbVeiculo.place(relx=0.01, rely=0.26, relwidth=0.15, relheight=0.15)

        self.listAut = Entry(self.FrameAut)
        self.listAut.place(relx=0.15, rely=0.27, relwidth=0.82, relheight=0.13)

        ###  Label e Entrada Marca
        lbMarca = LabelGlac(self.FrameAut, 'Marca')
        lbMarca.configure(bg='gray20')
        lbMarca.place(relx=0.01, rely=0.42, relwidth=0.15, relheight=0.15)

        self.listMarca = Entry(self.FrameAut)
        self.listMarca.place(relx=0.15, rely=0.43, relwidth=0.82, relheight=0.13)

        ###  Label e Entrada Combustivel
        lbFuel = LabelGlac(self.FrameAut, 'Combust')
        lbFuel.configure(bg='gray20')
        lbFuel.place(relx=0.01, rely=0.58, relwidth=0.15, relheight=0.15)

        self.listCombustivel = Entry(self.FrameAut)
        self.listCombustivel.place(relx=0.15, rely=0.59, relwidth=0.82, relheight=0.13)
